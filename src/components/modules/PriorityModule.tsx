...
import { TrendingUp, ArrowUpDown, ShieldAlert, BarChart3, HelpCircle } from 'lucide-react';
    ...
    <div className="space-y-4">
      ...
      <div className="p-4 rounded-xl bg-gov-card border border-gov-border">
        ...
        <p className="text-xs text-slate-300 max-w-4xl mt-1 leading-relaxed">
          Why does Mundakkai move before Vellarmala? The priority engine evaluates 5 transparent, auditable factors rather than opaque AI weights. Every rank is backed by measurable geomorphic and life-safety indicators.
        </p>
      </div>

      {/* Priority Table with Factor Breakdown */}
      <div className="bg-gov-dark rounded-xl border border-gov-border overflow-hidden">
        <div className="p-3 border-b border-gov-border flex items-center justify-between font-bold text-xs text-slate-300">
          ...
          <span className="text-slate-400 text-[11px]">Ranked by Composite Life-Safety Threat</span>
        </div>
          ...
          <table className="w-full text-xs text-left">
            <thead className="bg-gov-surface text-slate-400 uppercase text-[10px] tracking-wider border-b border-gov-border">
              <tr>
                ...
                <th className="py-2.5 px-3">Recurrence (35%)</th>
                <th className="py-2.5 px-3">Red Zone (25%)</th>
                <th className="py-2.5 px-3">Access Risk (15%)</th>
                <th className="py-2.5 px-3">Slope (10%)</th>
            ...
            <tbody className="divide-y divide-gov-border/60">
              {evaluated.map((item, idx) => {
                ...
                return (
                  <tr key={s.id} className="hover:bg-gov-surface/50">
                    <td className="py-3 px-3">
                      <div className="flex items-center gap-2">
                        <span className="w-5 h-5 rounded-full bg-slate-800 text-slate-200 font-mono font-bold flex items-center justify-center text-[10px]">
                          #{idx + 1}
                        </span>
                        <div>
                          <span className="font-bold text-white block">{s.name}</span>
                          <span className="text-[10px] text-slate-400">{s.localPanchayat}</span>
                        </div>
                      </div>
                    </td>
                    <td className="py-3 px-3">
                    ...
                    <td className="py-3 px-3 font-mono text-slate-300">
                      {s.population} ({s.households} HH)
                    </td>
                    <td className="py-3 px-3">
                      <div className="space-y-1">
                        <div className="font-mono text-slate-200">{s.factors.hazardRecurrence}/100</div>
                        <div className="w-16 h-1 bg-slate-800 rounded-full overflow-hidden">
                          <div className="h-full bg-red-500" style={{ width: `${s.factors.hazardRecurrence}%` }} />
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="py-3 px-3">
                      <span className="text-base font-black font-mono text-red-400 px-2 py-0.5 rounded bg-slate-900 border border-slate-700">
                        {b.calculatedScore}
                      </span>
                    </td>
                  </tr>
      ...
      <div className="p-4 rounded-xl bg-gov-card border border-gov-border space-y-3">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-2 text-xs">
            <select
              ...
              className="bg-gov-surface text-slate-200 border border-gov-border rounded px-2 py-1"
            >
              {settlements.map(s => (
        ...
        <div className="p-3 rounded-lg bg-gov-surface border border-gov-border text-xs space-y-2">
          <div className="flex items-center gap-2">
            <span className="font-bold text-white text-sm">Decision Conclusion:</span>
          ...
          <div className="space-y-1 pt-1">
            <span className="text-slate-400 text-[11px] uppercase tracking-wider font-bold">Key Contributing Factor Differentials:</span>
            {comparison.differentialFactors.length > 0 ? (
              ...
              <p className="text-slate-400 text-xs">Equal threat index across comparative dimensions.</p>
            )}
          </div>
        </div>
      </div>
